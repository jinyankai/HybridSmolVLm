        ...
        for layer_idx in  range(len(self.layers)):
            if layer_idx not in self.attn_layers:
                ...
                if init_with_qkv:
                    ...
                    # fc1 ← gate_proj
                    mamba_mlp.gate_proj.weight.data.copy_(llama_mlp.gate_proj.weight.data)
                    if mamba_mlp.gate_proj.bias is not None and llama_mlp.gate_proj.bias is not None:
                        mamba_mlp.gate_proj.bias.data.copy_(llama_mlp.gate_proj.bias.data)

                    # fc2 ← up_proj
                    mamba_mlp.up_proj.weight.data.copy_(llama_mlp.up_proj.weight.data)
                    if mamba_mlp.up_proj.bias is not None and llama_mlp.up_proj.bias is not None:
                        mamba_mlp.up_proj.bias.data.copy_(llama_mlp.up_proj.bias.data)

                    # fc3 ← down_proj
                    mamba_mlp.down_proj.weight.data.copy_(llama_mlp.down_proj.weight.data)
                    if mamba_mlp.down_proj.bias is not None and llama_mlp.down_proj.bias is not None:
                        mamba_mlp.down_proj.bias.data.copy_(llama_mlp.down_proj.bias.data)
                    ...
                    mamba_decoder.self_attn_mamba.out_proj.weight.data.copy_(
                        llama_layer.self_attn.o_proj.weight.data
                    )

                    # ---------- 3) 输入投影 (q,k,v → in_proj) ----------
                    ip_w = mamba_decoder.self_attn_mamba.in_proj.weight.data
                    ...
                    if mamba_config.is_bias:
                        mamba_decoder.self_attn_mamba.out_proj.bias.data.copy_(llama_layer.self_attn.o_proj.bias.data)

                        ip_b = mamba_decoder.self_attn_mamba.in_proj.bias.data
                ...
                self.model.model.text_model.layers[layer_idx] = mamba_decoder
